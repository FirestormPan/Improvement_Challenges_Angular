...
Users have a profile picture that can be changed(no other info, for simplicity). 
...
Completing a contract creates a new card in the database with input derriving from the contract. The contract is deleted from the database. 

### Cards
A card includes a challenge. The owner of the card can activate the card "on" a participant of the contract which created the card. Completing the card removes it from the database
